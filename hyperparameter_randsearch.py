...
### MODEL PARAMETERS ###
embedding_size_list = [16, 32, 64, 128]
hidden_size_list = [16, 32, 64, 128]
learning_rate_list = [1e-4, 1e-3, 1e-2, 0.1, 1]

vocab_size = len(vocab)
target_size = 5
embedding_size = choice(embedings_size_list)
hidden_size = choice(hidden_size_list)
learning_rate = choice(learning_rate_list)
...
def scoring_fn(last_state):
    ...
    sents = tf.transpose(tf.pack(sentences_encoded, axis=1), perm=[0, 1, 2])  # batch_size x 5 x hidden_size
...
with tf.Graph().as_default():
    ...
    sentence_lens = tf.unstack(story_sent_lens, num=5, axis=1)
    ...
    sentences_encoded_packed = tf.pack(sentences_encoded, 1)

    # TODO: maybe prefix with start_sent?
    ...
    for dec_out_train, dec_out_test in zip(dec_outputs_train[:-1], dec_outputs_test[:-1]):
        ...
        # score = tf.Print(score, [tf.shape(score), score], 'Score: ')
    ...
    with tf.Session() as sess:
        ...
        model_name = 'e_size' + str(embedding_size) + 'h_size' + str(hidden_size) + 'lr' + str(learning_rate)
        ...
        with open('./model/hyperparameter-results') as file:
